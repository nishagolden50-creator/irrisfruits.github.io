...
function renderSales(){$('#sales-list').innerHTML=sales.map(x=>row(`${x.product_name} × ${x.quantity}`,`${fmt(x.created_at)} · ${x.payment_method}${x.mpesa_code?' · '+x.mpesa_code:''}`,money(x.quantity*x.unit_price),`<button class="mini" onclick="showReceipt('${x.id}')">Receipt</button>${can('sales')?`<button class="mini" onclick="editSale('${x.id}')">Edit</button>`:''}`)).join('')||empty('No sales recorded yet')}
